 ...
 * by 1 if it's positive (otherwise, if its value is 0, then it's left unchanged), and then push it back onto the queue
 ...
 * in the original array. The remaining elements are then decremented (aside from the 0) and pushed backed onto the
 * queue, whose contents are then [0, 0, 0, 1].
...
public class QueueRemoval {
    ...
    int[] findPositions(int[] arr, int x) {
        ...
        for (int i = 0; i < sN; i++) {
            ...
            if (!isMaxRemoved && tempElement == max) {
                isMaxRemoved = true;
            } else {
                q.add(tempElement - 1);
            }

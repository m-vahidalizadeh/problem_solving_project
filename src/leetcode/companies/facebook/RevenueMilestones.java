 ...
 * is a length-K array of leetcode.interesting total revenue milestones.
 * Output
...
public class RevenueMilestones {
    ...
    int[] getMilestoneDays(int[] revenues, int[] milestones) {
        Map<Integer, Integer> originalOrders = new HashMap<>();
        int mIndex = 0;
        int revIndex = 0;
        int n = revenues.length;
        int m = milestones.length;
        for (int i = 0; i < m; i++) {
            originalOrders.put(milestones[i], i);
        }
        Arrays.sort(milestones);
        int[] resArray = new int[m];
        if (revenues[0] == milestones[0]) {
            resArray[originalOrders.get(milestones[0])] = 1;
            mIndex++;
        }
        revIndex++;
        while (revIndex < n) {
            revenues[revIndex] += revenues[revIndex - 1];
            while (mIndex < m && revenues[revIndex] >= milestones[mIndex]) {
                resArray[originalOrders.get(milestones[mIndex])] = revIndex + 1;
                mIndex++;
            }
            revIndex++;
        }

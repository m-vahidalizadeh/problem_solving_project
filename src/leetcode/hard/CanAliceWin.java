...
public class CanAliceWin {
    ...
    public boolean canAliceWin(String[] a, String[] b) {
        int ai = 0, bi = -1, an = a.length, bn = b.length;
        while (true) {
            for (bi++; bi < bn; bi++) {
                if (isCloselyGreater(b[bi], a[ai])) {
                    while (bi + 1 < bn && b[bi + 1].charAt(0) == b[bi].charAt(0)) bi++;
                    break;
                }
            }
            if (bi == bn) return true;
            for (ai++; ai < an; ai++) {
                if (isCloselyGreater(a[ai], b[bi])) {
                    while (ai + 1 < an && a[ai + 1].charAt(0) == a[ai].charAt(0)) ai++;
                    break;
                }
            }
            if (ai == an) return false;
        }
    }

    private boolean isCloselyGreater(String a, String b) {
        if (a.compareTo(b) <= 0) return false;
        return a.charAt(0) <= b.charAt(0) + 1;
    }

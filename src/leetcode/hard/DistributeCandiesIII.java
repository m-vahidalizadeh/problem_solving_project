...
public class DistributeCandiesIII {
    ...
    public long distributeCandies(int n, int limit) {
        return getCount(1, n, limit, new HashMap<>());
    }

    private long getCount(int index, int rem, int limit, HashMap<Pair<Integer, Integer>, Long> cache) {
        if (index == 3) {
            if (rem > limit) return 0;
            else return 1;
        }
        Pair<Integer, Integer> key = new Pair<>(index, rem);
        if (cache.containsKey(key)) return cache.get(key);
        long res = 0;
        for (int take = 0; take <= rem && take <= limit; take++) {
            res += getCount(index + 1, rem - take, limit, cache);
        }
        cache.put(key, res);
        return res;
    }

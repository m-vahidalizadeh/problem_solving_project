...
public class FindShortestPathWithKHops {
    ...
    public int shortestPathWithHops(int n, int[][] edges, int s, int d, int k) {
        ...
        while (!q.isEmpty()) {
            int[] curr = q.poll();
            if (mark[curr[2]][curr[1]]) continue;
            mark[curr[2]][curr[1]] = true;
            if (curr[2] == d) return curr[0];
            if (graph[curr[2]] == null) continue;
            for (int[] nei : graph[curr[2]]) {
                if (nei[1] + curr[0] < dist[nei[0]][curr[1]]) {
                    dist[nei[0]][curr[1]] = nei[1] + curr[0];
                    q.add(new int[]{dist[nei[0]][curr[1]], curr[1], nei[0]});
                }
                if (curr[1] > 0 && curr[0] < dist[nei[0]][curr[1] - 1]) {
                    dist[nei[0]][curr[1] - 1] = curr[0];
                    q.add(new int[]{dist[nei[0]][curr[1] - 1], curr[1] - 1, nei[0]});
                }
            }
        }
        return -1;

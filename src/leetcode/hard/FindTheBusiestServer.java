    ...
    public List<Integer> busiestServers(int k, int[] arrival, int[] load) {
        ...
        for (int i = 0; i < arrival.length; i++) {
            ...
            if (free.contains(i % k)) {
                ...
                busy.add(i % k);
                freq[i % k]++;
                max = Math.max(max, freq[i % k]);
            } else {
                Integer next = free.ceiling(i % k);
                if (next == null) next = free.first();
                free.remove(next);
                availability[next] = arrival[i] + load[i];
                busy.add(next);
                freq[next]++;
                max = Math.max(max, freq[next]);
            }
        }
        List<Integer> res = new ArrayList<>();
        for (int i = 0; i < k; i++) {
            if (freq[i] == max) res.add(i);
        }
        return res;
    }

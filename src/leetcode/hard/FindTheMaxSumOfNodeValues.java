 ...
 */
public class FindTheMaxSumOfNodeValues {

    public long maximumValueSum(int[] nums, int k, int[][] edges) {
        long res = 0;
        int minDiff = Integer.MAX_VALUE, isOddXORs = 0;
        for (int num : nums) {
            int xorNum = num ^ k;
            res += Math.max(num, xorNum); // Add num or its XOR to the res
            isOddXORs ^= xorNum > num ? 1 : 0; // Update if you are doing XOR
            minDiff = Math.min(minDiff, Math.abs(xorNum - num)); // Keep track of min diff if the number of XORs become odd at the end
        }
        return res - minDiff * isOddXORs; // We can do even number of XORs. Let's skip the min diff node XOR.
    }

...
public class LatestDayToCross {
    ...
    private boolean canReach(int row, int col, int[][] cells, int day) {
        int[][] grid = new int[row][col];
        for (int i = 0; i < day; i++) grid[cells[i][0] - 1][cells[i][1] - 1] = 1;
        Queue<int[]> q = new ArrayDeque<>();
        for (int c = 0; c < col; c++) {
            if (grid[0][c] == 0) {
                q.add(new int[]{0, c}); // if it is not water
                grid[0][c] = 1; // visited
            }
        }
        while (!q.isEmpty()) {
            ...
            for (int[] dir : directions) {
                int newR = r + dir[0];
                int newC = c + dir[1];
                if (newR < 0 || newR >= row || newC < 0 || newC >= col || grid[newR][newC] == 1) continue;

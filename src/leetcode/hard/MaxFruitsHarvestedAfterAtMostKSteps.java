 ...
 * 0 <= k <= 2 * 105
 */
public class MaxFruitsHarvestedAfterAtMostKSteps {
    ...
    private boolean isValidRange(int leftPos, int rightPos, int startPos, int k) {
        if (rightPos <= startPos) return startPos - leftPos <= k;
        else if (startPos <= leftPos) return rightPos - startPos <= k;
        else { // leftPos < startPos < rightPos
            int left = startPos - leftPos;
            int right = rightPos - startPos;
            return left <= right ? 2 * left + right <= k : 2 * right + left <= k;
        }
    }

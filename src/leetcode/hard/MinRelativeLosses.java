 ...
 * 1 <= mi <= n
 */
public class MinRelativeLosses {

    public long[] minimumRelativeLosses(int[] prices, int[][] queries) {
        Arrays.sort(prices);
        int pn = prices.length, qn = queries.length, ind[] = new int[qn], startOfRight = 0;
        long[] sumArr = new long[pn + 1], result = new long[qn];
        for (int i = 1; i <= pn; i++) sumArr[i] = sumArr[i - 1] + prices[i - 1];
        for (int i = 0; i < qn; i++) ind[i] = i;
        ind = Arrays.stream(ind).boxed().sorted((a, b) -> queries[a][0] - queries[b][0]).mapToInt(i -> i).toArray();
        for (int i : ind) {
            int k = queries[i][0], m = queries[i][1];
            while (startOfRight < pn && prices[startOfRight] <= k) startOfRight++;
            int pickedFromLeft = Math.min(m, startOfRight), left = 1, right = Math.min(pickedFromLeft, pn - m);
            while (left <= right) {
                int mid = (left + right) / 2;
                if (prices[pickedFromLeft - mid] > 2 * k - prices[pn - (m - pickedFromLeft) - mid]) left = mid + 1;
                else right = mid - 1;
            }
            pickedFromLeft = pickedFromLeft - (left - 1);
            long pickedFromRight = m - pickedFromLeft;
            result[i] = pickedFromRight * 2 * k - (sumArr[pn] - sumArr[pn - (int) pickedFromRight]) + sumArr[pickedFromLeft];
        }
        return result;
    }

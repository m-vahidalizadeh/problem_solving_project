...
public class ModifyGraphEdgeWeights {
    ...
    public int[][] modifiedGraphEdges(int n, int[][] edges, int source, int destination, int target) {
        ...
        for (int i = 0; i < n; i++) adjs[i] = new HashMap<>();
        for (int[] edge : edges) {
        ...
        if (distTo[destination] == target) return fill(edges);
        else if (distTo[destination] < target) return new int[0][0];
        else {
            for (int[] edge : edges) {
                if (edge[2] == -1) {
                    edge[2] = 1;

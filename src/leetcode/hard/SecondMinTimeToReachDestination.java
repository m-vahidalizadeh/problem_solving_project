...
public class SecondMinTimeToReachDestination {
    ...
    public int secondMinimum(int n, int[][] edges, int time, int change) {
        ...
        for (long[] I : dist) Arrays.fill(I, Long.MAX_VALUE);
        ...
        while (!q.isEmpty()) {
            p = q.poll();
            wait = 0l;
            if ((p.val / change) % 2 == 1) wait = change - (p.val % change);
            val = p.val + wait + (long) time;
            for (int i : list[p.idx]) {
                if (dist[i][0] > val) {
                    dist[i][0] = val;
                    q.offer(new Duo(i, val));
                } else if (val > dist[i][0] && dist[i][1] > val) {
                    if (i == n) return (int) val;
                    dist[i][1] = val;
                    q.offer(new Duo(i, val));
                }
            }

...
public class VideoSharingPlatform {
    ...
    class Video {
        public int likes, dislikes, views, size;
        public String video;

        public Video(int likes, int dislikes, int views, String video, int size) {
            this.likes = likes;
    ...
    public int upload(String video) {
        int videoId = availableIds.isEmpty() ? currentId++ : availableIds.poll();
        videos.put(videoId, new Video(0, 0, 0, video, video.length()));
        return videoId;
    }
    ...
    public String watch(int videoId, int startMinute, int endMinute) {
        ...
        video.views++;
        return video.video.substring(startMinute, Math.min(video.size, endMinute + 1));
    }

    public void like(int videoId) {
        if (videos.containsKey(videoId)) videos.get(videoId).likes++;
    }

    public void dislike(int videoId) {
        if (videos.containsKey(videoId)) videos.get(videoId).dislikes++;
    }

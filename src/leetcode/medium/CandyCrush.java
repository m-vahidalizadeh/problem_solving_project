...
public class CandyCrush {
    ...
    public static void main(String[] args) {
        ...
        int[][] result = c.candyCrush(board);
        leetcode.base.Utils.printArray(result);
    }
    ...
    public int[][] candyCrush(int[][] board) {
        ...
        while (true) {
            ...
            for (int j = 0; j < m; j++) {
                ...
                for (int i = n - 1; i >= 0; i--) {
                    // If this is non-zero, shift it to the bottom.
                    if (board[i][j] > 0) swap(j, i, lastNonZeroCellRow--, board);
                }
            }
        }
        return board;
    ...
    private void swap(int j, int from, int to, int[][] board) {
        int temp = board[from][j];
        board[from][j] = board[to][j];
        board[to][j] = temp;
    }

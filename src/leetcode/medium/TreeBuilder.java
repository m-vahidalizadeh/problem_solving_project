...
public class TreeBuilder {
    ...
    public class OperatorNode extends Node {
        ...
        public OperatorNode(String val, Map<String, BiFunction<Integer, Integer, Integer>> operatorMap) {
            this.val = val;
        ...
        @Override
        public int evaluate() {
            return operatorMap.get(val).apply(left.evaluate(), right.evaluate());
        }
    }
    ...
    Node buildTree(String[] postfix) {
        ...
        for (int i = 0; i < postfix.length; i++) {
            if (operatorMap.containsKey(postfix[i])) {
                Node right = s.poll();
                Node left = s.poll();
                Node operator = new OperatorNode(postfix[i], operatorMap);
                operator.right = right;
                operator.left = left;
                s.push(operator);
            } else s.push(new IntegerNode(postfix[i]));
        }
        return s.poll();
    }
}
